...
​    2.使用角度：ChatGPT擅长于处理一些繁琐、但不会特别有深度、专业性的问题，帮助提高日常办公效率。比如写一篇文案、写一段简单的数据处理的代码、总结一段文本等等。但是ChatGPT只是帮助人们解放了80%的劳动力，其生成的内容还需要人们去校验与后处理。此外，其内部的知识在没有调用外部向量库的情况下没法做到实时更新，即知识有时效性；其还有幻觉现象，会一本正经地胡说八道，当外行人请教其一个专业点的知识时，根本不知道其说的到底可不可靠。

$\text{2.请简述下Transformer基本流程}$

​	Transformer由一个编码器和一个解码器构成，编码器是堆叠的transformer block构成。首先是最底层的输入，文本先被分成成token序列，$\text{token1,token2,...,tokenk}$，然后经过$\text{embedding layer}$得到$(\mathbf x_1,\mathbf x_2,...,\mathbf x_k)$，此外每一个token还有其对应的位置编码帮助模型编码位置信息，记作$(p_1,p_2,...,p_k)$，二者相加作得到$(\mathbf x_1+p_1,\mathbf x_2+p_2,....,\mathbf x_k+p_k)$为最底层transformer block的输入。编码器中的transformer block由多头自注意力机制$\text{multihead self-attention}$（编码器的自注意力机制没有掩码）和$\text{add \&layer normalization}$还有$\text{feed-forward neural network}$构成。多头自注意力机制帮助模型关注不同的语义信息，编码器的输出是序列化的语义向量。

​    编码器同样也是由transformer block堆叠而成，其最底层的输入是target token的词嵌入和位置编码，和编码器不同之处在于，其注意力机制是交叉注意力机制，即注意力机制的query来自解码器的语义表征，而key和value来自于编码器输出的语义表征。并有注意力机制掩码将对角线右上方的区域置为0，让模型不能获取未来时刻的信息，防止信息泄露。
...
​		$\text{1.采样：}$采样就是从一堆prompt池中选择一些prompt，然后模型基于这些prompt推理得到token的概率分布进行token采样得到最后的response，在这一步，我们不只需要采样到的response，我们还需要每一步的对数概率叫做$\text{old\_log\_prob}$，还有$\text{old\_value}$（生成token时评论家预估的收益）。假设我们有$M$个prompt,每一个prompt对应的response是token数为$N$的字符串，那么另外两个输出是：

1. Actor的输出$\text{actor\_log\_prob}$: $M\times N$的张量，其中每一个prompt对应于一个$1 \times N$的张量，张量里的元素是$p(token|context)$。
1. Critic的输出$\text{values:}$$M \times N$的张量，其中每一个prompt对应于一个$1 \times N$的张量，包含了每次生成token时评论家预估的收益。

​		采样这个环节可以理解为学生在黑板上答题，学生只留下了答案，还未得到老师的反馈和指导。

​		$\text{2.反馈：}$反馈就相当于老师检查学生答案的过程，会给学生的回答(response)进行打分，即有一个奖励模型(Reward Model)用于衡量prompt和Actor模型输出的response的匹配程度，prompt和response会拼起来给奖励模型打分得到一个标量score。但这个分数不是最终的奖励，因为score只能说明结果的好坏不能说明过程的合理性，那怎样的过程是合理的？1.循规蹈矩至少不会太差，即和SFT阶段已经训练好的模型（Reference Policy）差不多，因此按照这个规矩来可以得到少量奖励 2. 给予最终的奖励前，对Actor的标新立异给予少量的惩罚（认为他在挑战权威）。通过KL散度可以衡量两个分布间的相似度，我们希望Actor的策略和Reference 相差Policy不要太大，因此通过KL散度来约束Actor策略，具体的：	

​												$\begin{aligned}reward[i]=  \left\{\begin{array}{ll}\text { ref } \_ \text {log } \_ \text {prob }[i]-\text { actor } \_ \text {log } \_ \text {prob }[i], & i<N \\
\text { ref } \_ \text {log } \_ \text {prob }[N]-\text { actor } \_ \text {log } \_ \text {prob }[N]+\text { score }, & i=N
\end{array}\right.   \end{aligned}$

​		最终获得的奖励则是$score-\beta KL(\pi_{ref}||\pi_{actor})$		

​		反馈环节就是老师在检查学生的答案并给出评价，学生们就可以了解他们的表现如何从而进行学习和改进。模型也需要通过学习阶段来提升其生成内容的质量。
...
​		最终的损失就是$\text { actor\_loss }+\beta \times \text { critic\_loss }$。
